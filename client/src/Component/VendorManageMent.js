...
console.log("vendorData",vendorData)
  ...
  const deleteVendorData = async (row) => {
    try {
      const response = await axios.delete(
        `http://admin.srimagicprintz.com/api/Vendor/vendorInfo/deletevendordata/${row._id}`
      );
  ...
  const handleAddGroup = async () => {
    try {
      const cleanedData = selectedVendorObjects.map((vendor) => {
        return {
          _id: vendor._id || "",
        };
      });
      ...
      const config = {
        ...
        data: { InstallationGroup: cleanedData },
      };
                          ...
                          >
                            <p>
                              {vendorData[selected].VendorImage ? (
                                <img
                                  ...
                                  src={`http://admin.srimagicprintz.com/VendorImage/${vendorData[selected].VendorImage}`}
                                  alt=""
                                />
                              ) : null}
                            </p>
                            ...
                            {vendorData[selected].BankInfoImage ? (
                              <img
                                width={"200px"}
                                height={"100px"}
                                src={`http://admin.srimagicprintz.com/BankInfoImage/${vendorData[selected].BankInfoImage}`}
                                alt=""
                              />
                            ) : null}
